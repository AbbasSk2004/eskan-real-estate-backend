...
const createConversation = async ({ userId, participantId, propertyId = null }) => {
  ...
  const mapped = mapConversation(conversation, userId, [], usersById);

  // Live-push the new conversation to the other participant so their list
  // updates without a manual refresh.
  const recipientId = p1 === userId ? p2 : p1;
  sendToUser(recipientId, 'new_conversation', mapped);
...
const sendMessage = async ({ userId, conversationId, content }) => {
  ...
  const mapped = mapMessage(message, sender);

  // Live-push the message to the other participant's open sockets so the
  // message renders without a re-fetch or drawer reopen.
  const recipientId = conversation.participant1Id === userId
    ? conversation.participant2Id
    : conversation.participant1Id;
  sendToUser(recipientId, 'new_message', mapped);
